DROP PROCEDURE IF EXISTS core.procedure_action_city;
CREATE OR REPLACE PROCEDURE core.procedure_action_city(
    p_session_id UUID,
    p_payload JSONB
)
    LANGUAGE plpgsql
...
BEGIN
    ...
    IF v_status = 'POSTED' THEN
        ...
        IF EXISTS (SELECT 1 FROM temporary.core_city WHERE city_id = v_city_id AND session_id = p_session_id) THEN
            ...
            INSERT INTO core.city (
                ...
                SELECT
                    ...
                    NOW(),
                    is_removed
                FROM temporary.core_city
                WHERE city_id = v_city_id AND session_id = p_session_id
                ON CONFLICT (city_id)
                    DO UPDATE SET
            ...
            DELETE FROM temporary.core_city
            WHERE city_id = v_city_id AND session_id = p_session_id;

        ELSE
            -- Opsional: Jika user kirim POSTED tapi di temporary tidak ada datanya
            RAISE EXCEPTION 'ERROR PROCEDURE: Data not found in temporary for ID % in this session.', v_city_id;
        END IF;

    -- 3. LOGIKA EDIT (Salin Master ke Temp as Draft)
    ELSIF v_status = 'EDIT' THEN
        INSERT INTO temporary.core_city (city_id,
                                            city_name,
                                            status,
                                            session_id,
                                            is_removed)
        ...
        ON CONFLICT (city_id) -- Gunakan composite key jika ada
            DO NOTHING;
