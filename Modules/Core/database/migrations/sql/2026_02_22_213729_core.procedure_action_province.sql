DROP PROCEDURE IF EXISTS core.procedure_action_province;
CREATE OR REPLACE PROCEDURE core.procedure_action_province(
    p_session_id UUID,
    p_payload JSONB
)
    LANGUAGE plpgsql
...
BEGIN
    ...
    v_province_id := p_payload ->> 'province_id';
    ...
    IF v_status = 'POSTED' THEN
        ...
        IF EXISTS (SELECT 1 FROM temporary.core_province WHERE province_id = v_province_id AND session_id = p_session_id) THEN
            ...
            INSERT INTO core.province (
                ...
                SELECT
                    ...
                    NOW(),
                    is_removed
                FROM temporary.core_province
                WHERE province_id = v_province_id AND session_id = p_session_id
            ...
            DELETE FROM temporary.core_province
            WHERE province_id = v_province_id AND session_id = p_session_id;

        ELSE
            -- Opsional: Jika user kirim POSTED tapi di temporary tidak ada datanya
            RAISE EXCEPTION 'ERROR PROCEDURE: Data not found in temporary for ID % in this session.', v_province_id;
        END IF;

    -- 3. LOGIKA EDIT (Salin Master ke Temp as Draft)
    ELSIF v_status = 'EDIT' THEN
        INSERT INTO temporary.core_province (province_id,
                                            province_name,
                                            status,
                                            session_id,
                                            is_removed)
        ...
        ON CONFLICT (province_id) -- Gunakan composite key jika ada
            DO NOTHING;
