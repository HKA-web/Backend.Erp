...
DECLARE
    v_id TEXT := p_payload ->> 'province_id';
BEGIN
    -- Validasi Locking: Cek jika sudah ada draft milik session lain
    IF EXISTS (SELECT 1 FROM temporary.core_province WHERE province_id = v_id AND session_id <> p_session_id) THEN
        RAISE EXCEPTION 'Data is being edited by another user.';
    END IF;

    INSERT INTO temporary.core_province (province_id, province_name, status, session_id, is_removed)
    SELECT province_id, province_name, 'DRAFT', p_session_id, FALSE
    FROM core.province WHERE province_id = v_id
...
BEGIN
    ...
    SELECT is_removed INTO v_is_removed FROM temporary.core_province WHERE province_id = v_id AND session_id = p_session_id;

    -- A. Snapshot Data Lama
    SELECT to_jsonb(t) INTO v_old_data FROM core.province t WHERE t.province_id = v_id;

    -- B. Move ke Master
    INSERT INTO core.province (province_id, province_name, status, is_removed, created_at, updated_at)
    SELECT province_id, province_name, 'POSTED', is_removed, NOW(), NOW()
    FROM temporary.core_province WHERE province_id = v_id AND session_id = p_session_id
    ON CONFLICT (province_id) DO UPDATE SET
        province_name = EXCLUDED.province_name,
        is_removed = EXCLUDED.is_removed,
        status = 'POSTED',
    ...
    SELECT to_jsonb(t) INTO v_new_data FROM core.province t WHERE t.province_id = v_id;

    INSERT INTO history.province_history (history_id, executed_by, action, old_data, new_data, executed_at)
    VALUES (
        gen_random_uuid(),
        v_user_id,
    ...
    -- D. Cleanup Temporary
    DELETE FROM temporary.core_province WHERE province_id = v_id AND session_id = p_session_id;
END;
$$;